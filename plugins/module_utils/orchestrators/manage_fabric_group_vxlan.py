...
class ManageFabricGroupVxlanOrchestrator(NDBaseOrchestrator):
    model_class: Type[NDBaseModel] = FabricGroupVxlanModel
    ...
    def query_all(self) -> ResponseType:
        ...
        filtered to only VXLAN fabric group types (category=fabricGroup, management.type=vxlan).
        """
        try:
            ...
            return [
                f
                for f in fabrics
                if f.get("category") == "fabricGroup" and f.get("management", {}).get("type") == "vxlan"
            ]
