...
class ManageEbgpFabricOrchestrator(ConfigActionsMixin, NDBaseOrchestrator):
    ...
    _FABRIC_TYPE_FILTER: ClassVar[str] = "vxlanEbgp"
    ...
    def _query_all_for_management_states(self) -> ResponseType:
        """Fetch all fabrics and filter to vxlanEbgp client-side."""
        api_endpoint = self.query_all_endpoint()
        result = self._request(path=api_endpoint.path, verb=api_endpoint.verb, not_found_ok=True)
        fabrics = result.get("fabrics", []) or []
        return [f for f in fabrics if f.get("management", {}).get("type") == self._FABRIC_TYPE_FILTER]
    ...
    def _query_all_for_gathered(self, gathered_filters) -> ResponseType:
        ...
        # fabric_name-only filters use the exact endpoint, bypassing Lucene
        # quoting issues with hyphens and special characters.
        lucene_filters = []
        ...
        for expression in expressions:
            ...
            for f in fabrics:
                if f.get("management", {}).get("type") != self._FABRIC_TYPE_FILTER:
                    continue
                name = f.get("name")
                if not name or name in seen_names:
                    continue
                seen_names.add(name)
                all_fabrics.append(f)
