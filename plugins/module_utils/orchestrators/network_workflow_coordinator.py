...
Replaces the following workflow handlers from the dcnm_network action plugin:
...
The coordinator is constructed inside nd_network.py after the strategy is
...
directly. For parent fabrics it:
  ...
  4. Builds child module_args per child fabric and re-invokes nd_network.
...
class NetworkWorkflowCoordinator:
    ...
    def _run_state_machine(self, module_args: dict, strategy: Optional[BaseNetworkStrategy] = None) -> dict[str, Any]:
        ...
        """
        return self._network_state_machine().run_basic(module_args, strategy=strategy)
    ...
    def _run_state_machine_with_attachments(
        ...
        """
        return self._network_state_machine().run(module_args, strategy=strategy, defer_deploy=defer_deploy)
