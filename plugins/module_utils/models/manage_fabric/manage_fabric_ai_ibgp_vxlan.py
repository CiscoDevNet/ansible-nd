...
class FabricAiIbgpVxlanModel(NDBaseModel):
    ...
    def validate_fabric_consistency(self) -> "FabricAiIbgpVxlanModel":
        """
        # Summary
        ...
        if self.management is not None and self.management.type != FabricTypeEnum.AIML_VXLAN_IBGP:
            raise ValueError(f"Management type must be {FabricTypeEnum.AIML_VXLAN_IBGP}")
