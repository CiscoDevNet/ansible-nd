...
class VxlanEbgpManagementModel(NDNestedModel):
    ...
    bgp_asn_auto_allocation: bool = Field(
        ...
        description=(
            "Automatically allocate and track BGP ASN for leafs, borders and border gateways "
            "in Multi-AS mode"
        ),
        default=True
    )
    bgp_asn_range: Optional[str] = Field(
        alias="bgpAsnRange",
        description=(
            "BGP ASN range for auto-allocation "
            "(minimum: 1 or 1.0, maximum: 4294967295 or 65535.65535)"
        ),
        default=None
    )
    bgp_allow_as_in_num: int = Field(
        alias="bgpAllowAsInNum",
        description="Number of occurrences of ASN allowed in the BGP AS-path",
        default=1
    )
    ...
    allow_leaf_same_as: bool = Field(
        ...
        description="Leafs can have same BGP ASN even when AS mode is Multi-AS",
        default=False
    )
    ...
    route_map_tag: int = Field(
        alias="routeMapTag",
        description="Tag for Route Map FABRIC-RMAP-REDIST-SUBNET. (Min:0, Max:4294967295)",
        default=12345
    )
    ...
    nve_loopback_id: int = Field(
        alias="nveLoopbackId",
        description=(
            "Underlay VTEP loopback Id associated with the Network Virtualization Edge (nve) interface"
        ),
        ge=0, le=1023, default=1
    )
    ...
    dns_collection: List[str] = Field(
        default_factory=lambda: ["5.192.28.174"],
        alias="dnsCollection",
    ...
    syslog_server_vrf_collection: List[str] = Field(
        ...
        description=(
            "Syslog Server VRFs. One VRF for all Syslog servers or a list of VRFs, "
    ...
    greenfield_debug_flag: GreenfieldDebugFlagEnum = Field(
        ...
        description=(
            "Allow switch configuration to be cleared without a reload when "
            "preserveConfig is set to false"
        ),
    ...
    next_generation_oam: bool = Field(
        ...
        description=(
            ...
            "to aid in trouble-shooting VXLAN EVPN fabrics"
        ),
        default=True
    )
    ...
    ngoam_south_bound_loop_detect_probe_interval: int = Field(
        ...
        description=(
            "Set Next Generation (NG) OAM southbound loop detection probe interval in seconds."
        ),
        default=300
    )
    ngoam_south_bound_loop_detect_recovery_interval: int = Field(
        ...
        description=(
            "Set the Next Generation (NG) OAM southbound loop detection recovery interval in seconds"
        ),
        default=600
    )
    ...
    aiml_qos: bool = Field(
        alias="aimlQos",
        description=(
            "Configures QoS and Queuing Policies specific to N9K Cloud Scale (CS) & Silicon One (S1) "
            "switch fabric for AI network workloads"
        ),
        default=False
    )
    aiml_qos_policy: AimlQosPolicyEnum = Field(
        ...
        description=(
            ...
            "User-defined allows for custom configuration."
        ),
    ...
    dlb: bool = Field(
        description=(
            ...
            "Note: Inter-Switch-Links (ISL) will be configured as DLB Interfaces"
        ),
        default=False
    )
