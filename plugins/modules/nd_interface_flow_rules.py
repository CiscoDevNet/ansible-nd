    ...
    if isinstance(nodes, list):
        ...
        for node_id in all_nodes_set:
            ...
            if check_node_input_id and node_id not in existing_nodes_set:
                ...
                node_input_formated = {"nodeId": node_input.get("node_id"), "nodeName": node_input.get("node_id")}
                ports_input_formated = [{"port": port} for port in node_input.get("ports", [])]
                if flow_rule_type in ["PHYSICAL", "PORTCHANNEL"]:
                    node_input_formated["portsList"] = ports_input_formated
                    nodes_to_add.append(copy.deepcopy(node_input_formated))
                    node_input_formated["operation"] = "ADD"
                ...
                node_to_delete["operation"] == "DELETE"
                ...
                if isinstance(ports_input, list):
                    existing_node = next((existing_node for existing_node in existing_nodes if existing_node["nodeId"] == node_id))[["portsList", "flowNodeUuid"]]
                    ...
                    all_port_set = existing_port_set.union(existing_port_set)
                    for port in all_port_set:
                        if port in ports_input and port not in existing_port_set:
                            ...
                            port_uuid = next((existing_port["flowPortUuid"] for existing_port in existing_ports if existing_port["subnet"] == port))
...
def main():
    ...
    argument_spec.update(
        ...
        flow_rule_status=dict(type="str", aliases=["status"]),
    ...
    flow_rule_status = nd.params.get("flow_rule_status")
    flow_rule_type = nd.params.get("flow_rule_type")
    ...
    # Modify Path for interface flow Rule
    ...
    for flow_rules_config in flow_rules_history:
        if flow_rules_config.get("name") == flow_rule:
            ...
            if len(flow_rules_config.get("nodesList", [])) >= 1:
                existing_nodes.extend(flow_rules_config["flowRuleAttributeList"])
    ...
    if state == "present":
        ...
        if flow_rule_config != nd.previous:
            ...
            if uuid:
                ...
                payload = {"flowRuleAttributeList": subnets_to_update, "nodesList": nodes_to_update}
