...
def main():
    ...
    nd_init = NDModule(module)
    # TODO: find a more flexible way to query all objects when init NDNetworkResourceModule
    init_all_data = nd_init.query_obj(path).get("localusers")
    nd = NDNetworkResourceModule(module, path, identifier_key, init_all_data)

    state = nd.params.get("state")
    config = nd.params.get("config")

    if state == "deleted":
        new_config = [{identifier_key: object.get("login_id")} for object in config]
        nd.delete_configs(new_configs=new_config)
        ...
        for object in config:
            payload = {
                ...
                identifier_key: object.get("login_id"),
        ...
        if state == "merged":
            nd.merge_configs(new_configs=new_config)
        elif state == "replaced":
            nd.replace_configs(new_configs=new_config)
        else:
            # TODO: Introduce an attribute to avoid deleting certains critical objects
            nd.override_configs(new_configs=new_config, unwanted=["admin", "github_ci"])
