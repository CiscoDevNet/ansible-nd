                ...
                payload["rbac"] = {
                    "domains": {
                        security_domain.get("name"): {
                            "roles": (
                                [USER_ROLES_MAPPING.get(role) for role in security_domain["roles"]] if
isinstance(security_domain.get("roles"), list) else []
                            )
                        }
        ...
        # unwanted = [["passwordPolicy", "passwordChangeTime"], ["userID"]]
        if state == "merged":
            nd.merge_configs(new_configs=new_config)
        elif state == "replaced":
            nd.replace_configs(new_configs=new_config)
        else:
            nd.override_configs(new_configs=new_config)
