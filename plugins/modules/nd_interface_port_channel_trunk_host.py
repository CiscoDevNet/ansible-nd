...
options:
  ...
  state:
    ...
    - Use O(state=gathered) to read all trunkPoHost port-channels across the fabric without making changes.
...
- name: Gather all trunkPoHost port-channels in the fabric
  cisco.nd.nd_interface_port_channel_trunk_host:
    fabric_name: my_fabric
    state: gathered
  register: gathered_port_channel_trunk_host
"""
...
gathered:
  description:
  - All trunkPoHost port-channels across the fabric.
