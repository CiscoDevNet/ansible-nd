...
DOCUMENTATION = r"""
---
module: nd_compliance_requirement_config_import
version_added: "0.3.0"
short_description: Manage import configuration type compliance requirements
description:
- Manage import configuration type on Cisco Nexus Dashboard Insights (NDI).
author:
- Akini Ross (@akinross)
options:
  insights_group:
  ...
  file:
    description:
    - Name of the import configuration file to upload.
    type: str
...
EXAMPLES = r"""
- name: Get all compliance import configuration type requirements
  cisco.nd.nd_compliance_requirement_config_import:
    insights_group: igName
    state: query
  register: query_results
- name: Get a specific compliance import configuration type requirement
  cisco.nd.nd_compliance_requirement_config_import:
    insights_group: igName
    name: complianceRequirementName
    state: query
  register: query_results
- name: Create compliance import configuration type requirement
  cisco.nd.nd_compliance_requirement_config_import:
    insights_group: igName
    name: complianceRequirementName
    sites:
    ...
    file: fileName.json
    state: present
- name: Delete compliance import configuration type requirement
  cisco.nd.nd_compliance_requirement_config_import:
    insights_group: igName
    name: complianceRequirementName
    state: absent
...
def main():
    ...
    argument_spec.update(file=dict(type="str"), allow_new_configuration_objects=dict(type="bool", default=False))
    ...
    requirements = [item for item in ndi.query_requirements(insights_group) if item.get("configurationType") == "JSON_XML_BASED_CONFIGURATION_COMPLIANCE"]
