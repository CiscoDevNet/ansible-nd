...
"""
Unit tests for `nd_interface_ethernet_trunk_host` module-level helpers.

Covers `validate_interface_names`, `validate_within_item_duplicates`, `validate_across_item_duplicates`, and the
integrated `expand_config` flatten step. Live ND interaction is exercised by the integration target.
"""
...
def test_expand_config_00000_flattens_to_one_record_per_interface():
    ...
    config = [
        {
            ...
            "config_data": {"network_os": {"policy": {"native_vlan": 100}}},
        }
    ]
    ...
    assert all(item["config_data"]["network_os"]["policy"]["native_vlan"] == 100 for item in result)
...
def test_expand_config_00001_split_configs_on_same_switch():
    ...
    - Two items, same switch, different `native_vlan` produces records that retain each VLAN
    ...
    config = [
        ...
        },
        {
            ...
            "config_data": {"network_os": {"policy": {"native_vlan": 200}}},
        },
    ]
    result = expand_config(config)
    assert len(result) == 2
    by_name = {item["interface_name"]: item for item in result}
    assert by_name["Ethernet1/1"]["config_data"]["network_os"]["policy"]["native_vlan"] == 100
    assert by_name["Ethernet1/2"]["config_data"]["network_os"]["policy"]["native_vlan"] == 200
