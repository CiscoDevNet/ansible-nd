...
def test_port_channel_access_orchestrator_00405() -> None:
    """
    # Summary

    Verify `query_all` scans every switch for `state: gathered`, returns only accessPoHost port-channels,
    and injects `switchIp` onto each retained interface.

    ## Test

    - Gathered state queries both switches without user configuration
    - Access and trunk port-channels plus a physical ethernet are returned by the controller fixtures
    - Only accessPoHost port-channels are retained
