...
def test_response_handler_nd_01394():
    """
    # Summary

    Verify a 207 `associations[]` envelope with a failing item is classified as failure and labelled by accessOrTorSwitchId.

    ## Test

    - A 207 ToR associate body with one success and one failure item sets success=False
    - The failing item is labelled by its accessOrTorSwitchId in error_message
    - The succeeding item does not appear in error_message
    ...
    instance.response = {
        ...
        "DATA": {
            "associations": [
                {"accessOrTorSwitchId": "9ABC1234DEF", "aggregationOrLeafSwitchId": "9WU9XPHL9SW", "message": "associated successfully", "status": "success"},
                {
                    "accessOrTorSwitchId": "9D93REB08X5",
                    "aggregationOrLeafSwitchId": "9WU9XPHL9SW",
    ...
    assert instance.result["success"] is False
    assert instance.error_message is not None
    assert "9D93REB08X5: Out of Range : DCNM-UUID-302030 Id [0] is not within the range of 1 and 4096" in instance.error_message
    assert "9ABC1234DEF" not in instance.error_message


def test_response_handler_nd_01395():
    """
    # Summary

    Verify a 207 `associations[]` envelope whose items all succeed is classified as success.

    ## Test

    - A 207 ToR associate body with only success items sets success=True
    ...
    instance.response = {
        ...
        "DATA": {
            "associations": [
                {"accessOrTorSwitchId": "9D93REB08X5", "aggregationOrLeafSwitchId": "9WU9XPHL9SW", "message": "associated successfully", "status": "success"},
            ]
        },
    }
    instance.verb = HttpVerbEnum.POST
...
def test_response_handler_nd_01396():
    """
    # Summary

    Verify a GET-shaped `associations[]` list body (accessPairBase items, no `status` key) is not a false positive.

    ## Test

    - A 200 GET accessAssociations body whose items carry switch ids but no status sets success=True
    ...
    instance.response = {
        ...
        "DATA": {
            "associations": [
                {"accessOrTorSwitchId": "9D93REB08X5", "aggregationOrLeafSwitchId": "9WU9XPHL9SW", "accessOrTorSwitchName": "site1-tor1"},
                {"accessOrTorSwitchId": "9ABC1234DEF", "aggregationOrLeafSwitchId": "9WU9XPHL9SW", "accessOrTorSwitchName": "site1-tor2"},
            ]
        },
    }
    instance.verb = HttpVerbEnum.GET
